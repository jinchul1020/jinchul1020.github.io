(() => {
  ...
  function drawPixelCharacter(x, y, color, enemy = false) { const px = x * grid; const py = y * grid; context.fillStyle = color; context.fillRect(px + 3, py + 3, 14, 14); context.fillStyle = enemy ? "#331a25" : "#251a2b"; context.fillRect(px + 6, py + 6, 3, 3); context.fillRect(px + 12, py + 6, 3, 3); context.fillStyle = enemy ? "#e78950" : "#f2b84b"; context.fillRect(px + 5, py + 13, 10, 3); }
  function draw() { context.fillStyle = "#0c0b11"; context.fillRect(0, 0, canvas.width, canvas.height); context.strokeStyle = "#171522"; context.lineWidth = 1; for (let x = 0; x < columns; x += 1) for (let y = 0; y < rows; y += 1) context.strokeRect(x * grid, y * grid, grid, grid); game.snake.slice().reverse().forEach((segment, index) => drawPixelCharacter(segment.x, segment.y, index === game.snake.length - 1 ? "#8dd35f" : "#4d9c63")); if (game.food) { context.fillStyle = "#f26b5e"; context.fillRect(game.food.x * grid + 5, game.food.y * grid + 5, 10, 10); context.fillStyle = "#f7f1d2"; context.fillRect(game.food.x * grid + 8, game.food.y * grid + 7, 2, 2); } game.enemies.forEach((enemy) => { if (enemy.phase === "active") drawPixelCharacter(enemy.x, enemy.y, "#d65c4e", true); else { context.fillStyle = "#f2b84b"; context.beginPath(); context.arc(enemy.x * grid + 10, enemy.y * grid + 10, Math.min(9, 3 + enemy.explosion / 180), 0, Math.PI * 2); context.fill(); } }); }
